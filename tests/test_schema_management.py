...
def test_coerce_integers_invalid(invoke_cli):
    ...
    _, stderr = invoke_cli(cmd, stdin=COERCION_TESTS_JSON_RAW, assert_nonzero_exit_code=True, capture_stderr=True,
                           use_subprocess=True)
...
def test_missing_column(invoke_cli):
    ...
    """
    cmd = ['csv:-', '--schema', '{id: int, name: str, date: int, test: str}', '-o', 'json:-']
    assert json.loads(invoke_cli(cmd, stdin=EXAMPLE_CSV_RAW)) == [
