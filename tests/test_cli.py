...
def invoke_cli(args, stdin=None):
    cmd = ['tableconv'] + args
...
def test_tsv_to_csv_files(tmp_path):
...
def test_tsv_to_csv_files_inferred_scheme(tmp_path):
    with open(f'{tmp_path}/test.tsv', 'w') as f:
        f.write(EXAMPLE_TSV_RAW)
...
def test_inferred_numbers_from_ascii_format():
...
#     assert stdout_lines.pop(0) == '/privat[...]lti_input0/test.tsv=> '
...
def test_full_roundtrip_file_adapters(tmp_path):
...
def test_sqlite_url_errors(tmp_path):
    with pytest.raises(subprocess.CalledProcessError) as exc:
        invoke_cli(['csv://-', '-o', 'sqlite://db.db'], stdin=EXAMPLE_CSV_RAW)
        assert 'Invalid SQLite URL' in exc.stderr
    with pytest.raises(subprocess.CalledProcessError) as exc:
        invoke_cli(['csv://-', '-o', 'sqlite:///tmp/db.db'], stdin=EXAMPLE_CSV_RAW)
        assert 'Invalid SQLite URL' in exc.stderr
