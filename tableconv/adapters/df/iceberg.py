...
class IcebergAdapter(Adapter):
    ...
    def load(uri, query):
        if query:
            raise InvalidParamsError("Querying is not currently supported for iceberg, no iceberg query engine is available in tableconv. Use -F instead to load into memory and query with duckdb.")
        ...
        if 'catalog_uri' in parsed_uri.query:
            ...
            if path.endswith('.metadata.json'):
                ...
                metadata_path = os.path.join(metadata_folder, max((i for i in os.listdir(metadata_folder) if i.endswith('.metadata.json'))))
                logger.info(f"Loading table metainfo from {metadata_path}")
                table = StaticTable.from_metadata(metadata_path)
            else:
                raise InvalidParamsError(f"{path} is invalid. Either use a catalog_uri, or pass a direct table .metadata.json file")
    ...
    def dump(df, uri):
        ...
        if if_exists not in ('append', 'replace', 'error'):
            raise InvalidParamsError(f"if_exists parameter must be one of: append, replace, error")
        namespace = parsed_uri.query.get('namespace', 'default')
        catalog = IcebergAdapter.load_catalog(parsed_uri)
